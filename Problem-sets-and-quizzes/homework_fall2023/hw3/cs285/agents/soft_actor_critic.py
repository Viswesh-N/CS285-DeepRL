...
class SoftActorCritic(nn.Module):
    def __init__(
        ...
        target_critic_backup_type: str = "min",  # One of "doubleq", "min", "redq", or "mean"
        # Soft actor-critic
    ...
    def q_backup_strategy(self, next_qs: torch.Tensor) -> torch.Tensor:
        ...
        if self.target_critic_backup_type == "doubleq":
            ...
            subset_size = min(3, num_critic_networks)  # You can adjust the subset size as needed
            indices = np.random.choice(num_critic_networks, subset_size, replace=False)
            next_qs = torch.min(next_qs[indices], dim=0)[0]
    ...
    def update_critic(
        ...
        with torch.no_grad():
            ...
            target_values: torch.Tensor = reward + self.discount*next_qs*(1-done.float())   # change here
    ...
    def entropy(self, action_distribution: torch.distributions.Distribution):
        ...
        action_samples = action_distribution.rsample(torch.tensor([self.num_actor_samples])) # change here
        return -action_distribution.log_prob(action_samples).mean(dim=0)
    ...
    def actor_loss_reinforce(self, obs: torch.Tensor):
        ...
        with torch.no_grad():
            ...
            action = action_distribution.rsample(torch.tensor([self.num_actor_samples]))
            assert action.shape == (
            ...
            q_values = self.critic(torch.tile(obs, (self.num_actor_samples,1,1)), action)
    ...
    def actor_loss_reparametrize(self, obs: torch.Tensor):
        ...
        # Note: Think about whether to use .rsample() or .sample() here...
        action = action_distribution.rsample(torch.tensor([self.num_actor_samples]))    # change here

        # TODO(student): Compute Q-values for the sampled state-action pair
        q_values = self.critic(torch.tile(obs, (self.num_actor_samples,1,1)), action)   # change here

        # TODO(student): Compute the actor loss
        log_probs = action_distribution.log_prob(action)
        loss = -(q_values+self.temperature*log_probs).mean()
    ...
    def update(
        ...
        for _ in range(self.num_critic_updates):
            critic_info = self.update_critic(obs = observations,
                                             action = actions,
                                             reward =  rewards,
